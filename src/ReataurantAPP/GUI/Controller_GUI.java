...
import ReataurantAPP.database.Manager;
import ReataurantAPP.database.OrderOld;
import ReataurantAPP.database.OrderDetail;
...
//     
    ...
    {
        OrderOld rOrder = cDatabase.findOrderByID(orderID);
    ...
    {
        OrderOld rOrder = cDatabase.findOrderByID(closeOrderID);
        if( currentUserID != rOrder.getStaffID())
        {
            setErrorMessage("You are not eligible to delete the order.\n(The order belonges to " + rOrder.getStaffName() + ")");
            return false;    
    ...
    {
        OrderOld rOrder = cDatabase.findOrderByID(cancelOrderID);
        if( currentUserID != rOrder.getStaffID())
        {
            setErrorMessage("You are not eligible to delete the order.\n(The order belonges to " + rOrder.getStaffName() + ")");
            return false;    
...
//    
    ...
    {
        Iterator<OrderOld> it = cDatabase.getOrderList().iterator();
        String          state;
        ArrayList<String> initData = new ArrayList<String>();
        String          output;
        ...
        while (it.hasNext()) {
            ...
            output = String.format("OrderOld ID:%4d  StaffName:%-20s  Total:$%5.2f State:%-8s\n",
